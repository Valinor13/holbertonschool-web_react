...
function getAllNotificationsByUser(userId) {
  ...
  for (const notification in result.entities.notifications) {
    if (result.entities.notifications[notification].author === userId) {
      for (const message in result.entities.messages) {
        if (message === result.entities.notifications[notification].context) {
          contextList.push(result.entities.messages[message]);
        }
      }
    }
  }
